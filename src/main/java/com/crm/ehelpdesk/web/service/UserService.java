...
public class UserService {
  ...
  public User registerUser(UserDTO userDTO, String password) {
    ...
    authorityRepository.findById(AuthoritiesConstants.USER).ifPresent(newUser::setAuthorities);
...
//          .forEach(managedAuthorities::add);
  ...
  public Page<UserDTO> getAllManagedUsers(Pageable pageable) {
    ...
    if (userAuthorities.contains("ROLE_ADMIN")) {
      ...
      return userRepository.findAllByOwnerAndAuthoritiesNotIn(pageable, login, new HashSet<>(Arrays.asList(new Authority("ROLE_SUPER_ADMIN")))).map(UserDTO::new);
    } else {
  ...
  public List<String> getAuthorities() {
    List<String> userAuthorities = SecurityUtils.getUserAuthorities();
    if (userAuthorities.contains(AuthoritiesConstants.SUPER_ADMIN)) {
      return authorityRepository.findAll().stream().map(Authority::getName).collect(Collectors.toList());
    } else {
      return authorityRepository.findByNameNotIn(
        new HashSet<>(Arrays.asList(AuthoritiesConstants.SUPER_ADMIN, AuthoritiesConstants.ADMIN)))
